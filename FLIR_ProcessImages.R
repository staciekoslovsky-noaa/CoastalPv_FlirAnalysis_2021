# Coastal Pv Surveys: Read OCR from FLIR Images
# S. Hardy, 15OCT2019
...
dir <- as.character(list.dirs(paste(wd, "Originals_FLIR/", year[1], sep = ""), full.names = TRUE, recursive = TRUE))
...
original1 <- data.frame(path = original[grep("Originals_FLIR/[0-9][0-9][0-9][0-9]/[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]/[A-Z][A-Z][A-Z]$", original$path), ], stringsAsFactors = FALSE)
original2 <- data.frame(path = original[grep("Originals_FLIR/[0-9][0-9][0-9][0-9]/[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]/[A-Z][A-Z][A-Z]/[A-Z][A-Z][A-Z]_[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][a-z]$", original$path), ], stringsAsFactors = FALSE)
...
for (i in c(1:nrow(original))){
  ...
  if (length(images) > 0){
    for (j in 1:length(images)){
      temp_ocr <- data.frame(ocr = tesseract::ocr(images[j], engine = allowed, HOCR = FALSE), stringsAsFactors = FALSE)
      temp_ocr$image_path <- images[j]
      data_ocr <- rbind(data_ocr, temp_ocr)
    }
  }
}
rm(original, temp_ocr, year, i, j, dir, images, wd)
...
write.csv(data_ocr, "C:/skh/CoastalPv_FLIR_OCR4Review_20191106_SKH.csv", row.names = FALSE)
